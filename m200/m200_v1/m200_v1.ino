...
//#define fromOPAMP 2                   // for temperature protection                 PD2
#define logicProbe_Pin 3                // for logic probe and frecuency measurments  PD3
//#define powerLed_Pin 4                  // enable power led                           PD4
...
bool overCurrentOPAMP = 0;                  // Input attenuator (valid at 1)
...
//   attachInterrupt(digitalPinToInterrupt(2), Safe, FALLING ); //for frecuency measurments
  ...
  //display.println(current_Adjust_SUM * 100);
...
void Safe() {
  ...
  if(!bitRead(PIND, 2)){ //para amortiguar el pulso de la interrupcion
    if(((millis()- pastTime) > 50) ){
      if(((millis()- pastTime) < 2500) ){
        bitClear(PORTB, PB3);// digitalWrite(mosfetEnable, LOW);
        bitSet(PORTB, PB4);//digitalWrite(vibrador, HIGH);
        overCurrentOPAMP = 1;
      }
...
//  if (((millis()- pastTime) > 100) && ((millis()- pastTime) < 500) ) { //corto cricuito seguro
//
//  //  overCurrentOPAMP = 1;
// //   bitClear(PORTB, PB3);// digitalWrite(mosfetEnable, LOW);
//    bitSet(PORTB, PB4);//digitalWrite(vibrador, HIGH);
...
void Voltimiter_logicProbe() {
  ...
  if(!overCurrentOPAMP)bitSet(PORTB, PB3);// digitalWrite(mosfetEnable, HIGH);
  ...
  x = 0.7304 * x + 0.2696 * z;//0.7304 * y + 0.2696 * u; fc=0.5;Hd=c2d(H,0.1)
  sumCurr = x;
  ...
  if ((Current < 0) && (Current > (-0.1))) Current = Current * (-1);
  ...
  if ((scopeP % 2) == 0) {                  //firat check if sound is needed
    ...
    Timer1.stop();
    display.setCursor(80, 50);                  // for voltage display
    display.setTextSize(1);                  // double the character,
    display.println(F("no bip"));
    digitalWrite(buzzer_Pin, LOW);
    previousV = 0;
  }
...
//digitalWrite(debug_Pin, HIGH);
  ...
  if (Current > 9 || Current < -9 || overCurrentOPAMP) {//  si se pasa de corriente
    overCurrentOPAMP = 0;
    bitClear(PORTB, PB3);// digitalWrite(mosfetEnable, LOW);
    bitSet(PORTB, PB4);//digitalWrite(vibrador, HIGH);
    overCurrentTimeProtection = 2000;
    
  } else {
    bitClear(PORTB, PB4);//digitalWrite(vibrador, LOW);
    overCurrentTimeProtection = 0;
  }
 // digitalWrite(debug_Pin, LOW);
  ...
  delay(overCurrentTimeProtection);
